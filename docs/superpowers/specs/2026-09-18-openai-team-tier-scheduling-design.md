...
4. 管理页面使用 v0.2.5 官方套餐标签：`team` 显示为 `Business Standard`，`self_serve_business_prolite` 显示为 `Business Premium`。
...
```text
PLUS/Business Standard -> PRO/Business Premium -> API Key -> COMPAT
```
...
### Business Premium（内部按 Team Pro 语义调度）
...
- Business Premium 沿用 PRO 的粘性规则，5 小时数据不触发释放。
...
- `self_serve_business_prolite`、`prolite`、`chatgptpro` 与 `pro` 返回 PRO 池。
...
沿用官方 v0.2.5 的统一套餐显示标签映射：

- `self_serve_business_prolite` 显示为 `Business Premium`。
- `team` 显示为 `Business Standard`。
...
- 其他未知套餐继续进入 `COMPAT`。
- 现有数据库无需迁移。
- 已保存的 Business Standard 与 Business Premium 账号在新版本启动后立即按新映射参与调度。
...
8. Business Premium 即使存在 100% 的 5 小时快照仍保持候选资格。
9. Business Premium 的调度阈值忽略 5 小时信号并保留 7 天信号。
10. Business Premium 的自动重置与自动暂停忽略 5 小时触发条件。
11. 调度决策指标分别把 Business Standard 记录为 PLUS、Business Premium 记录为 PRO。

### 前端测试

1. `self_serve_business_prolite` 显示为 `Business Premium`。
2. 编辑账号后仍保存 `self_serve_business_prolite` 原始值。
3. `team` 显示为 `Business Standard`。
4. 未知套餐继续原样显示。
...
2. Business Premium 在所有 PLUS/Business Standard 候选耗尽或失效后、API Key 之前参与选择。
3. Business Premium 的 5 小时值不会触发调度暂停或自动重置。
4. 数据库中的原始 `plan_type` 保持 `team` 与 `self_serve_business_prolite`。
5. 管理页面显示 `Business Standard` 与 `Business Premium`，不再直接展示 `self_serve_business_prolite`。
