<template>
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="input-label">{{ t('payment.admin.keyQuotaUsd') }}</label>
          <input v-model.number="planForm.key_quota_usd" type="number" step="0.01" min="0" class="input" />
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.keyQuotaUsdHint') }}</p>
        </div>
      </div>
      <div class="grid grid-cols-2 gap-4">
...
const planForm = reactive({ name: '', group_id: null as number | null, description: '', price: 0, original_price: 0, key_quota_usd: 0, validity_days: 30, validity_unit: 'days', sort_order: 0, for_sale: true })
...
watch(() => props.show, (visible) => {
  if (!visible) return
  if (props.plan) {
    Object.assign(planForm, { name: props.plan.name, group_id: props.plan.group_id, description: props.plan.description, price: props.plan.price, original_price: props.plan.original_price || 0, key_quota_usd: props.plan.key_quota_usd || 0, validity_days: props.plan.validity_days, validity_unit: props.plan.validity_unit || 'days', sort_order: props.plan.sort_order || 0, for_sale: props.plan.for_sale })
    planFeaturesText.value = (props.plan.features || []).join('\n')
  } else {
    Object.assign(planForm, { name: '', group_id: null, description: '', price: 0, original_price: 0, key_quota_usd: 0, validity_days: 30, validity_unit: 'days', sort_order: 0, for_sale: true })
...
function buildPlanPayload() {
  ...
  return {
    ...
    key_quota_usd: planForm.key_quota_usd || 0,
