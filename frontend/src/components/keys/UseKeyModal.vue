...
function generateOpenAIFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const providerName = generateCodexProviderName()
  const model = selectCodexCatalogModel('gpt-5.5')
  const reasoningEffortLine = codexReasoningEffortTomlLine(model) ||
    (codexCatalogModelSlugs.value.length === 0 ? 'model_reasoning_effort = "medium"\n' : '')
...
function generateCodexProviderName(): string {
  // Keep the historical go2me provider for the default local config. Once a
  // routed catalog is fetched, use the native OpenAI provider name so the
  // catalog-selected model is accepted by Codex; API-key mode also uses it.
  return codexAuthMode.value === 'api-key' || codexModelManifestContent.value.trim() !== ''
    ? 'OpenAI'
    : 'go2me'
}
