...
function generateOpenAIFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  // config.toml content
  const configContent = `model_provider = "${generateCodexProviderName()}"
model = "gpt-5.5"
...
[model_providers.${generateCodexProviderName()}]
name = "${generateCodexProviderName()}"
base_url = "${baseUrl}"
wire_api = "responses"
...
function generateCodexProviderName(): string {
  return codexAuthMode.value === 'api-key' ? 'OpenAI' : 'go2me'
}
