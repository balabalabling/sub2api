...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	// Responses 的 image_generation 工具可能把图片模型放在 tools 内，
	// 此时 requestedModel 仍是文本模型，路由查表必须使用显式图片模型。
	routingModel := modelRoutingLookupModel(ctx, requestedModel, platform)
	var routingAccountIDs []int64
	if group != nil && routingModel != "" &&
	...
	if strictModelRoutingNoFallback(ctx, platform) && len(routingAccountIDs) > 0 {
		return nil, ErrNoAvailableAccounts
	}
...
// modelRoutingLookupModel returns the model key used by a group's routing
// table. Responses image_generation can carry a dedicated model while the
// top-level request model remains a text model.
...
// strictModelRoutingNoFallback keeps dedicated OpenAI image requests inside a
// configured image route instead of silently selecting a text-only account.
...
func (s *GatewayService) routingAccountIDsForRequest(ctx context.Context, groupID *int64, requestedModel string, platform string) []int64 {
	routingModel := modelRoutingLookupModel(ctx, requestedModel, platform)
	if groupID == nil || routingModel == "" || !modelRoutingAppliesToTargetPlatform(platform) {
		return nil
	}
...
func (s *GatewayService) selectAccountForModelWithPlatform(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, platform string) (*Account, error) {
	...
	if len(routingAccountIDs) > 0 {
		...
		if strictModelRoutingNoFallback(ctx, platform) {
			return nil, ErrNoAvailableAccounts
		}
	}
...
func (s *GatewayService) selectAccountWithMixedScheduling(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, nativePlatform string) (*Account, error) {
	...
	if len(routingAccountIDs) > 0 {
		...
		logger.LegacyPrintf("service.gateway", "[ModelRouting] No routed accounts available for model=%s, falling back to normal selection", requestedModel)
		if strictModelRoutingNoFallback(ctx, nativePlatform) {
			return nil, ErrNoAvailableAccounts
		}
	}
