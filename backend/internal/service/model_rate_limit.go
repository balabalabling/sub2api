...
// WithOpenAIImageGenerationRoutingModel stores the explicit image model from a
// Responses image_generation tool so model routing can select the matching
// account rule while the original request model remains unchanged.
func WithOpenAIImageGenerationRoutingModel(ctx context.Context, model string) context.Context {
	...
	return context.WithValue(ctx, ctxkey.OpenAIImageGenerationRoutingModel, model)
}

// OpenAIImageGenerationRoutingModelFromContext returns the explicit image
// model attached to a Responses request, if any.
func OpenAIImageGenerationRoutingModelFromContext(ctx context.Context) string {
	if ctx == nil {
		return ""
	}
	model, _ := ctx.Value(ctxkey.OpenAIImageGenerationRoutingModel).(string)
